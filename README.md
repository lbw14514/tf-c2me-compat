...
C2ME 是区块生成性能优化 mod。它自带的密度函数编译器（dfc，含 JVM 字节码和 OpenCL GPU 加速两条路径）不认识暮色森林的自定义地形函数，轻则暮色部分地形无法加速，重则开启 OpenCL 后**一创建世界就崩溃**。本 mod 把暮色森林的自定义地形函数逐个"翻译"给 C2ME，让两边协同工作。
...
MC 26.2 + Fabric Loader 0.19.5 + C2ME 0.4.2-alpha.0.43 + OpenCL 模块（Quadro P620）+ 暮色森林 4.8.4208：主世界/下界/末地 OpenCL 正常运行，暮色森林维度按设计优雅回退，长时间游玩无崩溃。
